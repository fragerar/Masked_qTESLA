...
int crypto_masked_sign(unsigned char *sm, unsigned long long *smlen,const unsigned char *m, unsigned long long mlen, masked_poly sk, masked_poly e, unsigned char* seed){
  ...
  while (1) {
    ...
    ctr_sign++;
#endif         
    DG(y, PARAM_B, W_ZERO);  // Sample y uniformly at random from [-B,B]
    ...
    for(int i=0; i < MASKING_ORDER+1; ++i) poly_add(z[i], y[i], Sc[i]);                         // Compute z = y + sc

    if(full_RS(z) == 0){ // Rejection sampling
#ifdef DEBUG
  rejyzctr++;
#endif
      continue;
  ...
  rejwctr++;
    ...
    for(int i=0; i<PARAM_N; ++i) {
      ...
      if (z_unmasked[i] > PARAM_Q/2) z_unmasked[i] -= PARAM_Q;
    }

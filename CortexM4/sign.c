...
void masked_hash(unsigned char *c_bin, masked_poly v, const unsigned char *hm){

  /*
   *  Input:
   *    v: masked poly
   *    hm: hash of the message
   *  Output:
   *    c_bin: hash of the (unmasked) rounding of v together with the message 
   */

  int shares[MASKING_ORDER+1];
  unsigned char t[PARAM_N+HM_BYTES];
  int i, j;

  for(i=0; i < PARAM_N; ++i){
    for(j=0; j < MASKING_ORDER+1; ++j) shares[j] = v[j][i];
    t[i] = masked_rounding(shares);
  }
...
int crypto_sign_open(unsigned char *m, unsigned long long *mlen, const unsigned char *sm, unsigned long long smlen, const unsigned char *pk)
{
  ...
  poly_sub_reduce_pot(w, w, Tc);                     // Compute w = az - tc
...
void crypto_masked_keypair(masked_poly sk, masked_poly me, unsigned char* pk, unsigned char* seed)
{
  ...
  int temp[MASKING_ORDER+1];
  for(int i=0; i < PARAM_N; ++i){
    temp[0] = s[i];
    for(int j=1; j < MASKING_ORDER+1; ++j) temp[j]=0;
    full_refresh_arith(temp,temp);
    for(int j=0; j < MASKING_ORDER+1; ++j) sk[j][i] = temp[j]; 


    temp[0] = e[i];
    for(int j=1; j < MASKING_ORDER+1; ++j) temp[j]=0;
    full_refresh_arith(temp,temp);
    for(int j=0; j < MASKING_ORDER+1; ++j) me[j][i] = temp[j]; 
...
int crypto_masked_sign(unsigned char *sm, unsigned long long *smlen,const unsigned char *m, unsigned long long mlen, masked_poly sk, masked_poly e, unsigned char* seed){
  ...
  randombytes(randomness_input, CRYPTO_RANDOMBYTES+CRYPTO_SEEDBYTES);
  SHAKE(randomness_input+CRYPTO_RANDOMBYTES+CRYPTO_SEEDBYTES, HM_BYTES, m, mlen);
  SHAKE(randomness, CRYPTO_SEEDBYTES, randomness_input, CRYPTO_RANDOMBYTES+CRYPTO_SEEDBYTES+HM_BYTES);
  ...
  while (1) {
    ...
    ctr_sign++;
#endif         
    DG(y, PARAM_B, W_ZERO);  // Sample masked y uniformly at random from [-B,B]

    for(int i=0; i < MASKING_ORDER+1; ++i) poly_mul_pot(v[i], a, y[i]);
    masked_hash(c, v, randomness_input+CRYPTO_RANDOMBYTES+CRYPTO_SEEDBYTES); 
    encode_c(pos_list, sign_list, c);           // Generate c = Enc(c'), where c' is the hashing of v together with m

    for(int i=0; i < MASKING_ORDER+1; ++i) sparse_mul32(Sc[i], sk[i], pos_list, sign_list);
    for(int i=0; i < MASKING_ORDER+1; ++i) poly_add(z[i], y[i], Sc[i]);                         // Compute z = y + sc on shares

    if(full_RS(z) == 0){ // Masked Rejection sampling
#ifdef DEBUG
  rejyzctr++;
#endif
      continue;
    ...
    for(int i=0; i < MASKING_ORDER+1; ++i) sparse_mul32(Ec[i], e[i], pos_list, sign_list);
    for(int i=0; i < MASKING_ORDER+1; ++i) poly_sub_correct(v[i], v[i], Ec[i]); 

    if (full_WR(v) == 0){ // Masked Well Rounded
#ifdef DEBUG
  rejwctr++;
#endif
      continue;
    ...
    full_add(z, z_unmasked);

    for(int i=0; i<PARAM_N; ++i) {
      z_unmasked[i] &=(PARAM_Q-1);
      if (z_unmasked[i] > PARAM_Q/2) z_unmasked[i] -= PARAM_Q; // output centered representative of the signature
    }
